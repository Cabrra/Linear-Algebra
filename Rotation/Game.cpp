...
//#include"Game3DModel.h"
 ...
 Planes ground;  // define the ground
 Cube   box1, box2; // define the 2 boxes 
 Pillar pillar1,pillar2,pillar3 ,pillar4, pillar5; // define the 5 pillars
...
bool cGame::InitializeGame()
{
	...
	// the device is Device and texture file is "desert.bmp"
	ground.CreatePlane(Device,"desert.bmp");
	 // create box1 by calling CreateCube(theDevice,textureFilename)
	// the device is Device and texture file name is "crate.jpg"
	...
	// create the box2 by calling CreateCube(theDevice,textureFilename)
	// the device is Device and texture file name is "crate.jpg"
	box2.CreateCube(Device, "crate.jpg");
	// set box 2  position at (10,5,-15)
	box2.position = D3DXVECTOR3(10.0f, 5.0f, -15.0f);
	// set box 2 constant velocity to (0,0,0) m/s
	box2.velocity = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	// create  pillar 1 with position at (-5,0,-1) and orientation of 90 degree
	// by calling CreatePillar(theDevice,x,y,z,angle)
	pillar1.CreatePillar(Device,-5,0,-1,D3DX_PI/2);
	// create  pillar 2 with position at (14,0,5) and orientation of 90 degree
	// by calling CreatePillar(theDevice,x,y,z,angle)
	pillar2.CreatePillar(Device,14,0,5,D3DX_PI/2);
    // create  pillar 3 with position at (-30,0,25) and orientation of 180 degree
	// by calling CreatePillar(theDevice,x,y,z,angle).
	pillar3.CreatePillar(Device,-30,0,25,D3DX_PI);
	// create  pillar 4 with position at (-75,0,-30) and orientation of -45 degree
	// by calling CreatePillar(theDevice,x,y,z,angle)
   	pillar4.CreatePillar(Device, -75, 0 , -30, -D3DX_PI/4);
	// create  pillar 5 with position at (25,0,-25) and orientation of 90 degree
	// by calling CreatePillar(theDevice,x,y,z,angle)
...
bool RenderFrame()
{
 	// render ground by calling Render(theDevice)
	ground.Render(Device);
	// render box1 and box2 by calling Render(theDevice)
	box1.Render(Device);
	box2.Render(Device);
	// render pillar 1 to pillar 5 by calling Render(theDevice)
...
INT WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInst,LPSTR lpCmdLine, int nShowCmd)
{
 ...
 game=new cGame();
  /* Create window */
 game->CreateGameWindows(hInstance," Linear Algebra Tutorial  : Rotations simulation");
